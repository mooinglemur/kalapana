...
Kalapana's own version is `VERSION` in `server/version.mjs`. The image build stamps in the commit it was built from (the `KALAPANA_COMMIT` build argument, which `.gitlab-ci.yml` fills from the pipeline), so the page footer reads `0.1.0+abc1234` (the commit as semver build metadata); a local run shows the number alone.
